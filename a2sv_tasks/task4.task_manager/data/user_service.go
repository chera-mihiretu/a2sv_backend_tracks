...
func (us *UserService) RegisterUser(user models.User) (models.User, error) {
	...
	if hasData := result.Next(context.TODO()); hasData {
		user.Role = "admin"
	} else {
		user.Role = "user"
	}
...
func (us *UserService) LoginUser(user models.User) (string, error) {
	...
	err = us.Collection.FindOne(context.TODO(), bson.M{"email": user.Email, "password": string(hashed_password)}).Decode(&newUser)
	if err != nil {
		return "", err
	}
	hashedUser := user
	hashedUser.Password = string(hashed_password)
	token, err := us.MiddleWare.GenerateToken(hashedUser)
